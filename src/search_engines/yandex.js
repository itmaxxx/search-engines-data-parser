...
async function parseQuery(query) {
		...
		for (let pageNum = 0; pageNum < 10; pageNum++) {
			...
			for (let i = 0; i < searchResults.length; i++) {
				...
				try {
					...
					let title = await (
						await el.findElement(By.css('.OrganicTitle-LinkText'))
					).getAttribute('innerText');
					let linkEl = await el.findElement(
						By.css('.Link.Link_theme_outer.Path-Item')
					);
			...
			console.log(
				`${pageLinksCount} links parsed from ${searchResults.length} found elements`
			);
			...
			let nextPageEl = await driver.findElement(
				By.css('.link.pager__item_kind_next')
			);
	...
	if (links.length) {
		// fs.writeFile(
		// 	`./outputs/${query.replace(/ /g, '_')}-yandex.json`,
